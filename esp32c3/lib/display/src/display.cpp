...
UWORD alignSegoe(const char *text, const sFONT *font, uint8_t percentage, bool horizontal)
{
    ...
    if (horizontal)
    {
        ...
        Serial.println((EPD_2IN9_V2_HEIGHT - length) * percentage / 100);
        return (uint16_t)(EPD_2IN9_V2_HEIGHT - length) * percentage / 100;
    }
    else
    {
        Serial.println((EPD_2IN9_V2_WIDTH - font->Height) * percentage / 100);
        return (uint16_t)(EPD_2IN9_V2_WIDTH - font->Height) * percentage / 100;
    }
}

UWORD alignMono(const char *text, const mFONT *font, uint8_t percentage, bool horizontal)
{
